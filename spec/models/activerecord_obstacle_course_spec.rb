...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '6. finds multiple orders by id' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # orders = Item.find(ids).index_by(&:id).slice(*ids)
  ...
  it '14. plucks all values from one column' do
    ...
    # ------------------------------------------------------------
    names = Item.distinct.pluck(:name)
    # ------------------ Using ActiveRecord ----------------------
    # ------------------------------------------------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:order_items, :orders).where("order_items.item_id = ?", item_8.id).distinct.pluck(:name)
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    names = Order.all.last.items.distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = Order.joins(:user).group(:user_id).order('name').select('users.name as name', 'count(*) as total_order_count')
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).group(:user_id).order(name: :desc).select('users.name as name', 'count(items.id) as total_item_count')
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(:items).group(:order_id).order('users.name DESC').select('users.name as user_name',:order_id, 'count(item_id) as item_count')
